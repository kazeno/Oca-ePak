    ...
    $.each(ocaRelayCarriers, function (idx, id_carrier) {
        ...
        function loadRelayList() {
            ...
            $.each(ocaStateRelays[0], function (i, relay) {
                $select.append($("<option>").attr('value', ocaStateRelays[0][i]['IdCentroImposicion']).text(
                    toTitleCase(relay.Sucursal)+' - '+toTitleCase(relay.Calle)+' '+toTitleCase(relay.Numero)+', '+toTitleCase(relay.Localidad)
                ));
            });
        ...
        function carrier_selection(e)
        {
            ...
            if ($elem.prop('checked')) {
                ...
                $elem.closest('.delivery-option').next('.carrier-extra-content').show();
                windowVisible[idx] = 1;
                ...
                $elem.closest('.delivery-option').find('.carrier-extra-content').hide();
                $elem.closest('.delivery-option').next('.carrier-extra-content').hide();
            }
            return null;
        }
...
(function autoInitOca() {
    if ((typeof ocaeInitialized !== 'undefined') && ocaeInitialized)
        return true;
    if ((typeof ocaRelayCarriers !== 'undefined') && (typeof customerAddress !== 'undefined') && (typeof ocaRelays !== 'undefined'))
        ocaeInit();
    else {
        setTimeout(autoInitOca, 500);
        console.log('waiting for Oca variables init')
    }
})();
